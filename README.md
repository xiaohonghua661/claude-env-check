...
- 脚本为 **UTF-8 无 BOM**（内部 chcp 65001）；**不要**加 BOM 或另存为 ANSI/GBK（BOM 会导致 cmd 报 `'@echo 不是内部或外部命令'` 无法运行）
...
- 记事本打开 .bat 若显示乱码，是「UTF-8 无 BOM」被按 GBK 误读的正常现象，请用 VS Code / Notepad++ 查看（不要用记事本另存为其他编码）
